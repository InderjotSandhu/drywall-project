...
export default function NewService() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const res = await fetch('/api/admin/services', {
      ...
      body: JSON.stringify({
        ...form, id: form.id || form.title.toLowerCase().replace(/\s+/g, '-'),
  ...
  return (
    <div style={{ maxWidth: 700 }}>
      <h1 style={{ marginBottom: 24 }}>New Service</h1>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          ...
          <div>
            ...
            <input style={inputStyle} value={form.id} onChange={e => setForm(f => ({ ...f, id: e.target.value }))} placeholder="Auto from title" />
          </div>
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={labelStyle}>Detail (long description)</label>
